...
def test_permissions_commands(monkeypatch):
    ...
    monkeypatch.setattr(c, "_get_permission_based_on_resource_async", MagicMock(return_value="r"))
    monkeypatch.setattr(c, "_get_permission_based_on_scope_async", MagicMock(return_value="s"))
    ctx = _ctx()
    c.all(ctx); c.permission_on_resource(ctx, permission_id="p1"); c.permission_on_scope(ctx, permission_id="p2")
