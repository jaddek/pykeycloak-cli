...
def test_sessions_commands(monkeypatch):
    ...
    for n in ["_all_async","_count_async","_stats_async","_offline_sessions_async","_user_sessions_async","_delete_session_by_id_async","_delete_all_sessions_async","_delete_users_sessions_async"]:
        monkeypatch.setattr(c, n, MagicMock(return_value=n))
    ctx = _ctx()
    c.all(ctx); c.count(ctx); c.stats(ctx); c.offline(ctx, user_id="u1"); c.user(ctx, user_id="u1")
    c.delete_by_id(ctx, session_id="s1"); c.delete_all(ctx); c.delete_users_sessions(ctx, user_id="u1")
