...
def test_roles_commands(monkeypatch):
    ...
    ctx = _ctx()
    c.roles(ctx)
    c.role(ctx, role_name="r")
    c.create(ctx, name="r")
    c.update(ctx, role_name="r")
    c.delete_by_id(ctx, role_id="00000000-0000-0000-0000-000000000000")
    c.delete_by_name(ctx, role_name="r")
    c.assign(ctx, user_id="u1", role_name="r")
    c.user_roles(ctx, user_id="u1")
    c.user_composites_roles(ctx, user_id="u1")
    c.user_available_roles(ctx, user_id="u1")
    c.unassign(ctx, user_id="u1", role_name="r")
